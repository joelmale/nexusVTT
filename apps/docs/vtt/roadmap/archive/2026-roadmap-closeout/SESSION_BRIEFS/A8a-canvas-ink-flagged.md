...
- T0: pixel-compare harness — script renders N sample drawings (export from a real session's IndexedDB or synthesize all 4 types × 3 styles) through BOTH paths to PNG (playwright screenshot per path), `pixelmatch` diff, threshold below 1% differing pixels excluding antialias edges.
...
- Pixel-compare harness report committed under `tools/ink-compare/` with results below 1% diff on the sample set.
