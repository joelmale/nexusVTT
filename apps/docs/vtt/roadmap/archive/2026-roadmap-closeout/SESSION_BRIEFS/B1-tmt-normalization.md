...
- Output schema (align with C1's manifest v2): `id`, `name`, `category`, optional creature, tags, source path, SHA-256, and dimensions. Dimensions via image headers (T0 — e.g. `image-size` npm package), not decoding full images.
...
- T1 residue classification: batched prompts (~100 filenames per call, mapping each filename to name, category, and tags), temperature-stable, results merged with `source:'t1-classified'` marker for the sample review.
