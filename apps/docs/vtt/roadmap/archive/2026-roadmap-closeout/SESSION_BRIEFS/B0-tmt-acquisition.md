...
- T0: `acquire.mjs` (download release archive → extract → per-file sha256 → raw-inventory.json + top-level `release.lock.json` containing tag, archive SHA-256, file count, and acquisition time).
