...
- Shared operational tooling lives at the repo root: `docker-compose*.yml` for local stacks, `run-tests.sh` and `test-stack.sh` for orchestration, and `scripts/` for helper utilities. Codex reference material lives in `../docs/codex/`.

## Build, Test, and Development Commands

- Start full stack locally: `docker compose up -d`; stop with `docker compose down`. For docs, run `npm run start:docs` from the repository root.
