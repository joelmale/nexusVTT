...
(function () {
  ...
  function captureCanvasFallback() {
    try {
      ...
      if (canvas && canvas.width > 0 && canvas.height > 0) {
        canvas.toBlob(function (blob) {
          if (blob && blob.size > 0 && window.parent !== window) {
            console.log('Dwellings Bridge: Canvas fallback export captured');
            lastExportTime = Date.now();
            exportPending = false;
            window.parent.postMessage(
              {
                type: 'DWELLINGS_EXPORT_READY',
                payload: {
                  blob: blob,
                  filename: 'dwellings_' + Date.now() + '.png',
                  mimeType: 'image/png',
                },
              },
              '*',
      ...
      console.warn('Dwellings Bridge: Canvas fallback capture failed:', e);
      exportPending = false;
    }
  }
  ...
  function triggerExport() {
    ...
    try {
      ...
      console.warn('Dwellings Bridge: dispatchEvent failed:', err);
    }
  ...
  function initializeBridge() {
    ...
    window.saveAs = function (blob, filename) {
      console.log('Dwellings Bridge: Intercepted saveAs call:', filename, blob.type);
      lastExportTime = Date.now();
      exportPending = false;
        ...
        window.parent.postMessage(
          {
            ...
            payload: {
              blob: blob,
              filename: filename || 'dwellings.png',
    ...
    // Announce ready
    if (window.parent !== window) {
      window.parent.postMessage({ type: 'DWELLINGS_BRIDGE_READY' }, '*');
    }
    ...
    window.addEventListener('message', (event) => {
      ...
      if (data.type === 'REQUEST_EXPORT' || data.type === 'generator/export-request') {
        console.log('Dwellings Bridge: Received export request from parent');
        triggerExport();
      } else if (data.type === 'EXECUTE_ACTION' && data.keyCode) {
        console.log('Dwellings Bridge: Executing action keyCode:', data.keyCode);
        try {
          ...
          if ([13, 83].includes(data.keyCode)) {
