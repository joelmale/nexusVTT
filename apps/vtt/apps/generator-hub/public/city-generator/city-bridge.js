...
(function () {
  ...
  function captureCanvasFallback() {
    try {
      ...
      if (canvas && canvas.width > 0 && canvas.height > 0) {
        canvas.toBlob(function (blob) {
          if (blob && blob.size > 0 && window.parent !== window) {
            console.log('City Bridge: Canvas fallback export captured');
      ...
      console.warn('City Bridge: Canvas fallback capture failed:', e);
      exportPending = false;
    }
  }
  ...
  function triggerExport() {
    ...
    try {
      ...
      console.warn('City Bridge: dispatchEvent failed:', err);
    }
    ...
    window.addEventListener('message', (event) => {
      ...
      if (data.type === 'REQUEST_EXPORT' || data.type === 'generator/export-request') {
        ...
        try {
          ...
          if ([13, 83, 67, 84, 80].includes(data.keyCode)) {
