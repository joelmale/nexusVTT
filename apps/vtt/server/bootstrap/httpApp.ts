...
export function createHttpApp({
  ...
  registerApiRoutes(app, db, assetsPath, getSocketManager);
  ...
  app.use(
    createHealthRouter({
      ...
      getSocketManager,
      port,
      getManifest: () => manifestStore.current,
    }),
  );
  ...
  app.use(
    ...
    createRulesCatalogRouter({
      docApiUrl,
      timeoutMs: process.env.RULES_CATALOG_TIMEOUT_MS
        ? Number(process.env.RULES_CATALOG_TIMEOUT_MS)
        : undefined,
      cacheTtlMs: process.env.RULES_CATALOG_CACHE_TTL_MS
        ? Number(process.env.RULES_CATALOG_CACHE_TTL_MS)
        : undefined,
    }),
  );
  app.use(
