...
export const CharacterCreationWizard: React.FC<WizardProps> = ({
  ...
  // The hand-off to the host: one character per submission, errors surfaced
  // in-wizard, and the player's answers kept on failure.
  const {
    submit: handleSubmit,
    isSubmitting,
    isSubmittingRef: submittingRef,
    error,
    setError
  ...
  // Never dismiss the wizard while a save is in flight.
  ...
  // it, and restore focus to whatever opened it on close.
  ...
  const renderStep = () => {
    ...
    if (creationData.edition === '2024') {
      switch (currentStep) {
        ...
        case 13: return <Step8Traits {...commonProps} onSubmit={() => handleSubmit(creationData)} isSubmitting={isSubmitting} submitLabel={submitLabel} />;
  ...
  return createPortal(
    <div
      ref={rootRef}
      className={`${CHARACTER_CREATOR_ROOT_CLASS} fixed inset-0 backdrop-blur-md flex items-start sm:items-center justify-center z-50 p-2 sm:p-4 overflow-y-auto`}
      data-theme={theme}
    ...
    >
      ...
      >
        ...
        {/* Error Display */}
        {error && (
          <div className="flex-shrink-0 mx-4 sm:mx-6 md:mx-8 mb-4">
            <div
              role="alert"
              aria-live="assertive"
              className="bg-red-900 border border-accent-red-dark rounded-lg p-4"
            >
