...
export const DrawingPropertiesPanel: React.FC<DrawingPropertiesPanelProps> = ({
  ...
  return (
    <AnchoredPropertiesPanel anchor={anchor} label={label}>
      <div
        className="w-[300px] border-2 border-white/20 rounded-lg shadow-[0_8px_32px_rgba(0,0,0,0.5),0_0_0_1px_rgba(255,255,255,0.1)] backdrop-blur-md animate-[slideInRight_0.3s_ease]"
        style={{
          background:
          ...
          {selectedDrawings.length > 1 && (
            <div className="property-group">
              <p className="text-sm text-[var(--glass-text,#e0e0e0)] opacity-70 m-0">
                {selectedDrawings.length} drawings selected. Use the delete
                button below to remove all selected drawings.
              </p>
            </div>
          )}
