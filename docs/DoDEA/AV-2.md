...
| Sync token             | SHA-256 hash of the canonical game-state projection; paired with `stateVersion` to fence stale writers.                          |
| Durable state ACK      | `game-state-ack` sent only after PostgreSQL commits the snapshot, sync token, version, and campaign scenes.                      |
| Compare-and-swap (CAS) | Conditional PostgreSQL update that accepts a canonical writer only while its observed token and version still match.             |
| Authoritative resync   | Full committed snapshot/token/version returned to a stale or invalid writer so it rebases without overwriting the winner.        |
