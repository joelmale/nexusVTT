...
flowchart LR
  ...
  frontend --> backend
  backend --> postgres
  backend -. fanout, presence, host lease .-> redis
  backend --> oauth
...
| Browser | Backend via Frontend | WebSocket JSON | Room/session events, durable state ACKs, patches/resync snapshots, chat, heartbeat |
| Backend | PostgreSQL           | SQL over TCP   | Rows, canonical snapshot CAS tuples, ordered events, Express sessions              |
| Backend | Redis                | Redis TCP      | Cross-replica fanout, expiring presence, and host-lease fencing                    |
