...
| GameSession    | `roomCode`, `hostId`, `coHostIds`, `campaignId`, `players`, `status`, `dmConnected`, canonical `gameState`, `syncToken`, `stateVersion`                 | `src/types/game.ts`, `server/types.ts`, `server/repositories/base.ts`         |
| Room           | `code`, `host`, `coHosts`, `players`, `connections`, `status`, `gameState`, `syncToken`, `stateVersion`, `entityVersions`                               | `server/types.ts`                                                             |
...
| State upload            | Full canonical snapshot or RFC 6902 patch with content-hash chain anchors.                           |
| State acknowledgement   | `game-state-ack` containing the token/version already committed in PostgreSQL.                       |
| State patch             | `game-state-patch` containing JSON Patch operations, base/new tokens, and committed version.         |
| State conflict recovery | `game-state-resync-required` containing reason plus the authoritative snapshot, token, and version.  |
