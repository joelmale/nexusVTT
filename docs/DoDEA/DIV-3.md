...
| `sessions` | `id UUID` | `joinCode`, `campaignId`, `primaryHostId`, `status`, `gameState JSONB` | `status` constrained to `active`, `hibernating`, or `abandoned`. |
...
| State delta | `{ "type": "game-state-patch", "data": { "patch": [...], "version": number } }` |
