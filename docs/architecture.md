...
```text
nexusVTT/
├── apps/vtt/src/                # React web application
...
├── apps/vtt/services/asset-service/ # Independent workspace and runtime
├── apps/vtt/shared/              # Runtime-validated cross-boundary contracts
├── apps/vtt/tests/integration/   # Cross-process and database tests
└── apps/vtt/scripts/             # Deterministic build/asset tooling
```
...
- `apps/vtt/shared/` must not import from `apps/vtt/src/`, `apps/vtt/server/`, or a workspace package.
- The server and asset service validate unknown JSON before exposing typed
  values to application code.
- HTTP endpoints live in `apps/vtt/server/routes`; `apps/vtt/server/index.ts` owns process and
...
`apps/vtt/public/assets/dice-box`, compares JSON semantically, and validates every theme
...
Static character rules live in `apps/vtt/src/data/character`; `dataManager.ts` contains
...
- Keep shared utility tests in `apps/vtt/tests/unit` and cross-boundary tests in
  `apps/vtt/tests/integration`.
...
Run `npm run test:ci` from `apps/vtt` before merging. CI rejects static import cycles, verifies
