...
describe('SessionRepository', () => {
  ...
  it('should activate session by join code successfully', async () => {
    const mockSession = { id: 'session-1', joinCode: 'ABC1', status: 'hibernating' };
    ...
    pool.query.mockResolvedValueOnce({ rows: [{ ...mockSession, status: 'active' }] } as QueryResult); // final getSessionByJoinCode
    ...
    const session = await repository.activateSessionByJoinCode('ABC1', 'host-1');
  ...
  it('should return null if session to activate is not found', async () => {
    ...
    const session = await repository.activateSessionByJoinCode('INVALID', 'host-1');
  ...
  it('should retrieve player character details successfully', async () => {
    ...
    const character = await repository.getPlayerCharacter('user-1', 'session-1');
  ...
  it('should return null if player has no character linked', async () => {
    pool.query.mockResolvedValueOnce({ rows: [{ characterId: null }] } as QueryResult);
